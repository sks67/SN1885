...
exponentsArr = np.array([7., 8., 9., 10., 12.])
ledArr = np.array([1.26, 1.21, 1.19, 1.17, 1.15])
phiArr = np.array([0.47, 0.52, 0.55, 0.57, 0.60])
tstArr = np.array([0.732, 0.605, 0.523, 0.481, 0.424])
rstArr = np.array([0.881, 0.788, 0.725, 0.687, 0.636])

def radVel(n0 = 1.0, e51 = 1.0, mej = 2.0, tsnap = 500., n=10.):
    whr = np.where(n==exponentsArr)[0]
    led = ledArr[whr]
    phi = phiArr[whr]
    tstar_st = tstArr[whr]
    rstar_st = rstArr[whr]
    ...
    #Solution for ejecta dominated phase, TM99
    coeff1 = (27./(4.*3.14))*(1./(n*(n-3)))*(led**(n-2)/phi)
    coeff2 = ((10./3.)*((n-5.)/(n-3.)))**((n-3.)/2.)
    coeff = (coeff1*coeff2)**(1./n)

    t_st0 = tstar_st*tch
    tstar = tsnap/tch
    x = (rstar_st**(5./2.) + np.sqrt(zeta)*(tstar - tstar_st))
    vstar = ((n-3)/n)*coeff*tstar**(-3./n) if tsnap<t_st0 else (2./5.)*np.sqrt(zeta)*x**(-3./5.)
    rstar = coeff*tstar**((n-3)/n) if tsnap<t_st0 else x**(2./5.)
    v = vstar*vch  #in km/s                                                                                                                  
    r = rstar*rch  #in par
...
def lightcurve(n0 = 1.0, e51 = 1.0, mej = 2.0, pp=2.5,  epse=0.0, tsnap=500.0, slope=10., sntype='cc',stop_vel200=False,stop_sblimit=False):
    ...
    #*****CONSTRUCTION ZONE***********
    r_ed, v_ed, t_st0 = radVel(n0, e51, mej, tsnap, n=slope) 
    #*********************************
...
def lightcurve_Full(n0 = 1.0, e51 = 1.0, mej = 2.0, epse=0.0, pp=2.5, slope=10., sntype='cc', plot_LightCurve=False, plot_Radius = False, plot_EjectaVelocity = False, return_vals = True, stop_vel200 = True, stop_sblimit = False):
    ...
    for ind_t,t in enumerate(tim):
         rad[ind_t], lum[ind_t], vel[ind_t] = lightcurve(n0=n0,e51=e51,mej=mej,epse=epse,pp=pp,tsnap=t,slope=slope,sntype=sntype,stop_vel200=stop_vel200, stop_sblimit=stop_sblimit)
