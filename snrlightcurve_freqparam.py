#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 
# Light Curve analysis of SN1885 and keeping ejecta energy and mass 
# as free parameters. 
#
# For M31, 
# 1) Distance = 785 Mpc McConnachie (2005), MNRAS, 356, 979                                                                                                
# 2) flux limit = 2.7 muJy/beam                                                                                                                          
# 3) beam size = 1 arcsec (3.81 pc)                                                                                                                    
# 4) frequency = 4.86 GHz                                                                                                                            
#
# MODIFICATION:
#
# This is a copy of snrlightcurve_caprioli.py, with frequency being
# a free parameter. This was created to make the 3-panel plot of 
# L-age for SN1885 and G1.9 for my committee. Might want to think 
# about making this the default light curve script.
#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
def isDetect(fluxLim=0.0027, beamSize=3.81, diamSNR=0., lumSNR=0.):
    # fluxlim is the 3sigma limit, where the rms sensitivity is 50 muJy/beam
    dist = 0.785 #Mpc
...
def lightcurve(n0 = 1.0, e51 = 1.0, mej = 2.0, pp=2.5,  epse=0.0, freq=1.4e9, tsnap=500.0,sntype='cc',stop_vel200=False,stop_sblimit=False, returnIfSedov=False):
    ...
    c5 = 9.68e-24
    c6 = 8.10e-41
    dist = 840.0*1000*3.086e18  #m33 distance, for now
    nu = freq 
    mu=1.4
    mp = 1.67e-24
...
#    if epsCR <= (1.5e-3*AlfMach0 + 0.06):
#        epsb_up = f*epsCR*((vv1/c)+(1.0/AlfMach0))
#    else:
#        epsCR = 1.5e-3*AlfMach0 + 0.06
#        epsb_up = f*epsCR*((vv1/c)+(1.0/AlfMach0))
...
#    gammam=1.0
    ...
    if returnIfSedov:
        if tsnap>=t_st0:
            return (r_ed, lluni, v_ed, True)
        else:
            return (r_ed, lluni, v_ed, False)

    return (r_ed, lluni, v_ed)
 
def lightcurve_Full(n0 = 1.0, e51 = 1.0, mej = 2.0, epse=0.0, pp=2.5, freq=1.4e9, sntype='cc', plot_LightCurve=False, plot_Radius = False, plot_EjectaVelocity = False, return_vals = True, stop_vel200 = True, stop_sblimit = False):
    ...
    for ind_t,t in enumerate(tim):
         rad[ind_t], lum[ind_t], vel[ind_t] = lightcurve(n0=n0,e51=e51,mej=mej,epse=epse,pp=pp,freq=freq,tsnap=t,sntype=sntype,stop_vel200=stop_vel200, stop_sblimit=stop_sblimit)
